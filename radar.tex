...
Por trás do processo de construção do mapa de votação há vários conceitos envolvidos. Inicialmente consideramos que parlamentares e as diferentes opções (\yea ou \nay) de cada votação possuem pontos ideais no espaço. Assim sendo, em uma votação um parlamentar define seu voto em função da localização de seu ponto ideal e dos pontos ideias das opções associadas à votação. Uma primeira abordagem seria dizer que o parlamentar vota na opção cujo ponto está mais próximo de seu ponto ideal. Mas em vez disso, usamos o conceito de \emph{função utilidade}, que atribui a cada ponto no espaço um valor. Quanto mais alto esse valor, maior é a probabilidade de que o parlamentar vote em uma opção associada a esse ponto do espaço. 

Duas premissas importantes usualmente aplicadas às funções utilidade são 1) as funções são de pico único, sendo que no ponto ideal do parlamentar a função tem seu valor máximo; 2) a função é simétrica, ou seja, o parlamentar é indiferente a duas opções igualmente distantes de seu ponto ideal.

A função utilidade possui uma parcela determinística e também uma parcela estocástica, que possibilita a modelagem de \emph{erros de votação}. Um erro de votação seria a ideia de que o parlamentar não votou de acordo com suas preferências políticas. Um erro pode ter acontecido no sentido de que o parlamentar pode ter avaliado erroneamente os atributos das opções que definiriam seus pontos no espaço. Mas o erro pode refletir o fato de que fatores não modelados foram decisivos naquela votação em específico.

O \emph{Optimal Classification} (OC) consiste em um processo iterativo que procura maximizar a proporção de classificações corretas em um mapa espacial de votações. Dada uma configuração inicial de um mapa de votações, primeiramente aplica-se um algoritmo que maximiza a classificação correta fixando os pontos e movendo os linhas. Em um segundo passo, fixa-se as linhas e move-se os pontos para maximizar a classificação correta. Esses dois passos são repetidos várias vezes até que o erro (proporção de classificações incorretas) estabilize. Os algoritmos empregados garantem que a cada passo o erro nunca aumenta. \Leo{relacionar função de utilidade com esses algoritmos}.
...
Após a construção do mapa podemos observar que normalmente alguns parlamentares caem do lado errado de algumas retas. Isso significa que se o leitor do mapa fosse reconstituir os votos dados pelos parlamentares em cada votação, ele se enganaria em alguns casos. Esses erros representam uma imperfeição do mapa construído. Contudo, é preciso estar atento à linguagem utilizada na literatura, que explica a situação em termos de erros de predição, como se o modelo não fosse capaz de predizer as votações. Fica o alerta pois em nosso caso não há predições no sentido de que funções de utilidade encontrada serão aplicada em novas votações que não foram utilizadas na construção do mapa.

\Leo{qualidade dos resultados observados com OC...}

No NOMINATE temos algumas premissas a mais sobre as funções de utilidade. A mais importante é de que a função utilidade é exponencial. Outra opção utilizada em outros trabalhos são funções quadráticas. Essa diferença diz respeito a como o parlamentar vai se comportar em relação a opções cada vez mais longes de seu ponto ideal. Com funções exponencias, as opção cada vez mais longe se tornam cada vez mais indistinguíveis, enquanto que na exponencial elas se tornam cada vez pior avaliadas.

Agora mais precisamente, dado um parlamentar $x$ e uma opção $z$, a função de utilidade utilizada pelo NOMINATE é $U(x,z) = \beta * e^{\frac{-(x-z)^2}{2}} + \varepsilon$. Quanto mais perto $z$ está de $x$, maior o valor de $U$, sendo U simétrica e de pico único. $\beta$ é o fator de ruído, que determina o peso da parcela determinística da função de utilidade. Já a parcela estocástica é $\varepsilon$ segue a distribuição de valor extremo do tipo I.

Dada toda essa modelagem, a ideia básica do NOMINATE segue uma lógica parecida com a do OC. No NOMINATE temos três grupos de parâmetros: pontos ideias dos parlamentares, pontos ideais das opções das votações envolvidas e os parâmetros das funções de utilidade. Dada uma configuração inicial de pontos ideias dos legisladores, aplicamos sucessivamente os três seguintes passos: 1) estimamos os parâmetros das votações com base nos parâmetros dos parlamentares; 2) baseado nos parâmetros das votações, os parâmetros dos parlamentares são refinados; 3) com base nos parâmetros dos legisladores e das votações, os parâmetros das funções de utilidade são estimados. Esses passos são repetidos até a convergência, que é quando o refinamento para de ter efeito e os mesmos valores são produzidos.

%The approach that Howard Rosenthal and I developed, which we dubbed NOMINATE, solves the estimation problem by dividing the parameters into three sets and estimating each set separately with the other two held fixed. Given reasonable starting values for the legislator ideal points, we can estimate the roll call parameters given these ideal points. Similarly, given estimates of the roll call parameters, we can obtain better estimates of the ideal points. The third set is the utility function parameters, which are estimated given both the legislator ideal points and the roll call outcome points. NOMINATE cycles through the three sets until they reproduce each other. 9 That is, at convergence each set is reproduced by the other two – the parameters no longer change.

\Leo{O que são os ``parâmetros'' dos pontos ideais, das votações e da função utilidade? Na descrição parece que faltou dizer que os pontos ideais dos parlamentares devem ser estimados com base nas funções utilidade.}
...
Controvérsias sobre quantidade de dimensões. Poole diz que 1; Heckman diz que 2. Polêmica sem conclusão definitiva, de acordo com McCarty.
