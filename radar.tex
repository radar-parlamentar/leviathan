...
Dados os pontos ideias associados a parlamentares e votações, uma primeira abordagem simplista para determinar o voto do parlamentar em uma votação seria dizer que o parlamentar vota deterministicamente na opção mais próxima de seu ponto ideal. Mas em vez disso, Poole utiliza o conceito de \emph{função utilidade}, que atribui a cada ponto no espaço um valor. Quanto mais alto esse valor nos pontos do espaço onde há uma opção associada, maior é a \emph{probabilidade} de que o parlamentar vote naquela opção.
...
Muitos trabalhos consideram que para a análise de votações nominais os modelos lineares são menos adequados do que os modelos não lineares, como o \nominate de Poole. Heckman e Snyder~\cite{heckman-snyder1997}, porém, \ugh{demonstram rigorosamente a equivalência de modelos lineares com os resultados obtidos por Poole}, tendo como principal vantagem a simplicidade e eficiência computacional dos métodos lineares.\Saulo{Quando diz que demonstram rigorosamente a equivalência, dá a entender que eles demonstram matematicamente que é exatamente a mesma coisa fazer um ou fazer o outro, de um ponto de vista teórico, que o resulado sempre será exatamente o mesmo, idêntico, no caso geral. Certamente não foi isso que eles demonstraram, porque um método linear significa uma função utilidade quadrática, e o método de Poole usa a gaussiana, e não tem como ser a mesma coisa. Eu não me lembro da conclusão exata desse paper de Heckman e Snyder, se for o caso posso dar uma olhada, mas eu acho que deve ser algo na linha de ``demonstram que os resultados dos modelos lineares são muito próximos daqueles obtidos por Poole''. Que achas, é isso mesmo? }
...
Ainda na Figura~\ref{fig:projecao_cp1}, o segmento de reta vermelho destaca o trecho de CP1 sobre o qual incidem as projeções dos pontos. \ugh{O tamanho desse segmento está relacionado com a variância} dos dados sobre a dimensão definida por CP1, sendo que a reta CP1 foi escolhida pela ACP justamente por fornecer a maior variância possível dos dados em uma dimensão. Se escolhermos qualquer outra reta pra definir uma nova dimensão, como $r2$ na Figura~\ref{fig:projecao_r2}, teremos uma variância menor do que a variância dos dados sobre CP1.\Saulo{Léo, será que essas retas vermelhas não vão causar confusão, ao misturar conceito de variância com conceito de diferença entre o valor máximo e o valor mínimo dos dados? Sei que é uma tentativa de simplificar a explicação, mas acho que o risco é muito grande de transmitir um conceito errado. Eu tiraria a linha vermelha. Quem estiver lendo e não souber o que é variância que procure no google. Com essa atitude acho que contribuiríamos mais. Outra coisa que me ocorreu, essa revista Leviathan não tem problema com cores? Em geral é bom manter o mínimo possível de páginas com cores.}
...
\begin{figure}[h]
  ...
  \caption{Nos mapas espaciais justapostos é possível observar o resultado do posicionamento de três partidos em função de seus centroides: PT (vermelho), PSDB (azul) e PMDB (laranja). No período analisado, PT, PSDB e PMDB contaram com 14, 13 e 21 senadores respectivamente, o que afeta o tamanho dos círculos representando os partidos.}
  \label{fig:centroide}
\end{figure}
...
Este viés pode parecer arbitrário, porém essa abordagem é consistente tanto com a ideia de que um voto nulo representaria uma indiferença do parlamentar quanto aos resultados \yea e \nay (o voto nulo é euclidianamente equidistante das duas alternativas) quanto da ideia de que ao não votar o parlamentar pode ter uma preferência contrária àquela que se imagina que será aprovada na votação, como em um boicote à votação. Em outras palavras, um parlamentar teria maior tendência de comparecer e não se abster nem obstruir a votação em propostas nas quais ele esteja inclinado a votar com a maioria. Esta hipótese pode parecer arbitrária, mas a alternativa de descartar estes dados seria equivalente a considerar que um voto nulo equivale a um parlamentar com preferência igual à preferência média da casa, o que também não deixa de ser arbitrário.
...
Como visto na seção \ref{sec:revbib}, os trabalhos em ciências políticas normalmente não buscam somente a elaboração de mapas espaciais de votações, mas também a elaboração de teorias e modelos que embasem esses mapas. Porém, toda teoria parte de algumas suposições, que no caso concreto podem ou não ser verdadeiras. Davis et al.~\cite{davis1970electoral}, por exemplo, pressupõe que todos os eleitores avaliam os candidatos sob o prisma dos mesmo assuntos. Poole, tanto no OC quanto no \nominate, pressupõe funções utilidade simétricas e de pico único. Já no \nominate, temos ainda que considerar que erros são independentes e identicamente distribuídos. Mesmo Heckman e Snyder~\cite{heckman-snyder1997}, que utilizam modelos lineares, se apoiam fortemente em modelos de escolha discretas, considerando que o legislador modela suas preferências de forma multidimensional e aplica uma função utilidade quadrática sobre esses valores para tomar sua decisão.

Nossa abordagem para a análise do espaço de votações é muito similar ao modelo de Heckman e Snyder, utilizando inclusive a análise de componentes principais. No entanto, pode-se considerar que há alguma diferenciação sutil. Não assumimos explicitamente algumas condições, como funções utilidade quadráticas ou que o parlamentar avalie as proposições sempre de acordo com o mesmo conjunto de características\Saulo{Leo, eu entendo o que você está querendo dizer aqui, que nosso trabalho tem um teor mais descritivo e menos teórico. Porém eu tenho a forte impressão de que, embora nós não façamos suposições explícitas de função utilidade quadrática, ou parte estocástica do modelo, ou conjunto de características, todas essas suposições estão implícitas no modelo, no sentido em que poderíamos reescrever a abordagem do zero partindo dessas premissas e demonstrar, agora sim rigorosa e matematicamente, que o resultado é exatamente o mesmo, que as equações que descrevem um comportamento sob funções utilidades quadráticas em espaços euclidianos arbitrários são as mesmas que descrevem a ACP, há uma equivalência. A função utilidade quadrática está relacionada com a distância euclidiana quadrática, que é nada menos que a variância, usada na ACP. Não tenho certeza absoluta, porque não fiz as contas para demonstrar, mas eu tenho essa sensação. Talvez alguém mais esperto que nós lendo o artigo pode achar que falamos besteira nessa parte. Não sei se isso é motivo para tirar, mas talvez ter cautela.}. Preferimos também focar no significado das distâncias entre os parlamentares no mapa espacial, sem valorizar possíveis significados das dimensões dos mapas produzidos. Dessa forma, acreditamos que nosso modelo tenha uma interpretação simples e intuitiva, de forma que nossa abordagem seja suficientemente útil e prática no apoio a análises conjunturais realizadas por analistas políticos. Além disso, a aplicação de nosso modelo nas casas brasileiras apresenta medidas de avaliação similares ao consagrado \wnominate, como veremos mais adiante.
...
A fórmula da PRE considera apenas uma votação. Para se avaliar um conjunto de votações, se utiliza a APRE (redução proporcional do erro agregado)~\cite{leoni02cdep}, definida para $N$ votações por:

\[APRE = \frac{\sum_{k=1}^{N}\left(\left(\textrm{votos da minoria}\right)_k-\left(\textrm{erros do modelo}\right)_k\right)}{\sum_{k=1}^{N}\left(\textrm{votos da minoria}\right)_k}\]

\subsection{Aplicação à ACP}

Para a aplicar essas medidas à ACP, precisamos definir um \emph{preditor}. A ACP por si só não define um ``valor esperado'' para o voto de um dado parlamentar em uma dada votação. Por isso, a definição do preditor utilizado e aqui apresentado é de nossa autoria.
...
Para $\widehat{x}_{ij} > 0$ o preditor prevê que o parlamentar $j$ vota \yea na votação $i$; para $\widehat{x}_{ij} < 0$ o modelo prevê voto \nay; e para $\widehat{x}_{ij} = 0$ o modelo prevê um voto arbitrário (para facilitar a reprodutibilidade dos resultados foi adotado \yea nestes casos). Este modelo prevê apenas votos \yea ou \nay, ou seja, não prevê a possibilidade de abstenções, obstruções ou ausências.
...
%O recurso de utilizar formas geométricas para distinguir partidos foi utilizado neste artigo, mas não no software do Radar Parlamentar. Como em uma aplicação web dispomos de recursos interativos, optamos no Radar Parlamentar por destacar membros de um partido definido pelo usuário quando há interação com o nome do partido.
...
\ugh{Este artigo não objetiva realizar uma comparação direta entre a ACP e \wnominate. Os valores produzidos pelo \wnominate foram colocados apenas para que o leitor tenha alguma referência para avaliar os resultados que encontramos para a ACP}\Saulo{Você quer dizer que não objetivamos mostrar mapas do NOMINATE e comparar os mapas ponto a ponto? Ok, isso é verdade. Mas não deixa de ser uma comparação quando colocamos lado a lado as medidas de adequação. Acho que daria para tirar essa frase, porque quando eu li pareceu que ela acabou desvalorizando um pouco nosso trabalho.}. Dessa forma, o \wnominate foi escolhido meramente por sua proeminência entre os métodos de produção de mapas espaciais de votações, mesmo critério utilizado por Leoni~\cite{leoni02cdep}.
...
Porém, Leoni~\cite{leoni02cdep} e Izumi~\cite{izumi2016senado} não chegam a mesma conclusão. Diferentemente, eles concluem pela unidimensionalidade do espaço legislativo da Câmara dos Deputados e do Senado. Essa diferença ocorre porque eles utilizam a taxa de classificação correta e a APRE como critérios para determinar a dimensionalidade do espaço legislativo.

\section{Conclusões}
\label{sec:conclusoes}

Neste artigo apresentamos em detalhes a abordagem utilizada pelo software Radar Parlamentar na produção de mapas espaciais de votações do legislativo brasileiro. Procurou-se apresentar nossa abordagem, os trabalhos relacionados e as relações entre o nosso e outros trabalhos de forma relativamente didática. Em um campo de estudo interdisciplinar localizado predominantemente na ciências sociais, julgamos pertinente a existência de trabalhos como este, em que se detalham os conceitos matemáticos, para facilitar a recepção de novos pesquisados ao campo de estudo de análise quantitativa de votações legislativas.
...
Além disso, é de se notar que nos trabalhos de Poole, origem dos algoritmos utilizados por Leoni e Izumi, interpreta-se o congresso norte-americano como uma realidade unidimensional, ao passo que o trabalho de Heckman e Snyder~\cite{heckman-snyder1997}, que também utilizada a ACP, interpretam esse mesmo congresso norte-americano como multidimensional. Ou seja, as diferenças nos critérios de dimensionalidade também afetam as interpretações sobre o congresso dos EUA.
