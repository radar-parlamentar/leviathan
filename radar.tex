...
\Leo{Com as considerações do Izumi, talvez não seja mais tão interessante comparar o PCA com o nominate... será q faz sentido comparar com o OC } \Saulo{Pode até ser, desde que tenha o pacote no R. Mas aí não vai ter muita graça, o OC é como o nome diz classificação ótima, eu acho que qualquer algoritmo vai perder para ele na comparação de classificação, se ele permitir usar os dados de abstenções/obstruções/ausências como nós usamos na PCA.}
...
Para melhor visualizar as mudanças nestas posições relativas é conveniente rotacionar o resultado de uma das análises em torno da origem, de tal forma que seja minimizada alguma medida das "distâncias" percorridas pelos partidos entre os períodos. A Figura~\ref{fig:rotacoes} ilustra esta idéia, em que o resultado da
PCA de 2011 foi rotacionado de $\theta$ = 160º. \\


FIGURA: Explicando rotações dos mapas de votações \\

\Leo{hum, agora pra continuar tenho que copiar várias fórmulas to texto original, do qual não achei o .tex}.
