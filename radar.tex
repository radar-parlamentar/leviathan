...
A função utilidade utilizada pelo \nominate é $U(x,o) = \beta * e^{\frac{-w(x-o)^2}{2}} + \varepsilon$, onde $(x-o)^2$ representa a distância euclidiana entre os pontos $x$ e $o$. Quanto mais perto $z$ está de $o$, maior o valor de $U$, sendo $U$ simétrica e de pico único. $\beta$ e $w$ são parâmetros da função utilidade, sendo $\beta$ o fator de ruído, que determina o peso da parcela determinística da função de utilidade. $\varepsilon$ é a parcela estocástica e segue a distribuição de valor extremo do tipo I\footnote{\todo{Explicar ``distribuição de valor extremo do tipo I''}} \Leo{Leoni disse: ``O termo $\epsilon$ segue a distribuição logística'' Izumo tb diz logística, mas falando sobre \wnominate. Checar se leoni tb não se referia ao \wnominate e se o \nominate é diferente nesta questão da distribuição dos erros.}.

No NOMINATE temos então três grupos de parâmetros: pontos ideias dos parlamentares (um $x$ para cada parlamentar), pontos ideais das opções das votações envolvidas (um $o_n$ e um $o_y$ para cada votação) e os parâmetros da função utilidade ($\beta$ e $w$). O objetivo do \nominate é encontrar os valores de todos esses parâmetros para que se possa desenhar o mapa espacial de votações.
...
\Leo{OC produz apenas politopos, mas \nominate produz distâncias... comentar sobre isso?}
...
\todo{IDEAL: funcional utilidade é quadrática e os erros seguem a distribuição normal.}
