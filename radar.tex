...
\todo{Abordagem bayesiana, função utilidade quadrática, Markov chain Monte Carlo, vantagens teóricas, vantagens computacionais, modelo extensível.}

\todo{O artigo de 2004 não cita o nome IDEAL... então fazer algo assim: Jackman etc desenvolveram método assim assado \cite{clinton2004ideal}. Esse método é chamado de IDEAL \cite{carroll2009comparing} }

\todo{Pontos ideias estimados muito similares ao nominate (mais similar doq comparado à hackman). O processo iterativo do IDEAL é similar ao NOMINATE, mas usando algoritmos diferentes. A PCA também pode ser utilizada como valor inicial para o IDEAL. Clinton et al. falam que IDEAL tem vantagem de desempenho, mas não mostram medidas. Carroll fala que verificou propriedade e desempenho IDEAL vs NOMINATE e que não encontrou grandes diferenças, mas também não mostrou medidas de tempo de execução.}

% carroll:
% Our simulations reveal no clear advantage of one method over the other in producing estimates for any particular legislature size or number of votes taken. Further, neither estimator appears to more robust than the other in obtaining estimates from the data-generating process assumed by the other method.
