...
Se forem mantidos apenas as $d \leq N$ primeiras componentes principais, a parte relevante da matriz de rotação, que chamaremos de $\mathbf{R}_{(d)}$, terá apenas $d$ colunas, enquanto que a matriz de scores, $\mathbf{Y}_{(d)}$, terá apenas $d$ linhas. $\mathbf{R}_{(d)}^{t}\cdot \mathbf{Y}_{(d)}$ será então a melhor aproximação de $\mathbf{X^{*}}$ que pode ser obtida com um modelo linear deste tipo com $d$ dimensões, onde ``a melhor aproximação'' se refere à minimização da soma dos quadrados das diferenças dos valores entre as as matrizes $\mathbf{X^*}$ e $\mathbf{R}_{(d)}^{t}\cdot \mathbf{Y}_{(d)}$. \Leo{Dúvida... na Seção~\ref{sec:avaliacao_acp}, temos $\widehat{\mathbf{X}} = \mathbf{R}_{(d)}^{t} \cdot \mathbf{Y}_{(d)} + \mathbf{C}$, o que me dá a impressão de que a minimização seria das diferenças de $\mathbf{X^*}$ e $\widehat{\mathbf{X}}$. Só achei meio estranho não ter o $C$ aqui, mas tá certo né? Essa reflexão surgiu em parte porque talvez o texto ficasse mais simples se a matriz $\mathbf{R}_{(d)}^{t} \cdot \mathbf{Y}_{(d)}$ fosse batizada com algum nome... que seria algo tipo o $\widehat{\mathbf{X}}$. Que nome vc daria para $\mathbf{R}_{(d)}^{t} \cdot \mathbf{Y}_{(d)}$?}.
...
\Leo{Nas referências tá aparecendo ``fulando and ciclano''... deveria ser em português. Pesquisando na net, um jeito pra resolver isso seria alterando o estilo de citação (ieeetr). Mas vou deixar isso pra depois de verificar se o nosso target exige algum estilo específico.}
