...
Este artigo está organizado da seguinte forma: A seção \ref{sec:formulacao} traz a formulação matemática que propomos para aplicação da análise de componentes principais ao problema de modelagem de votações legislativas; \ref{sec:revbib} apresenta uma breve revisão de literatura;  \ref{sec:metodo} introduz os indicadores de \emph{fitness} utilizados na comparação dos métodos; \ref{sec:resultados} contém os resultados obtidos \ins{com tais e tais dados}; \ref{sec:discussao} apresenta uma discussão dos resultados e \ref{sec:conclusoes} conclui. \ins{etc... etc...}
...
\section{O Modelo ACP para Análise de Votações}
